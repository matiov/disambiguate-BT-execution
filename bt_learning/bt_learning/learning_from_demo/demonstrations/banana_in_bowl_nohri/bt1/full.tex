...
\begin{tikzpicture}
	\graph[tree layout] {
		n1[as=$\rightarrow$, control] -> {
			n2[as=?, control] -> {
				n3[as={banana roughly at $p_{1}$ in bowl?}, condition],
				n4[as=$\rightarrow$, control] -> {
					n5[as=?, control] -> {
						n6[as={In gripper banana?}, condition],
						n7[as=$\rightarrow$, control] -> {
							n8[as=?, control] -> {
								n9[as={Gripper open?}, condition],
								n10[as={Set gripper open}, action]
							},
							n11[as={Pick banana}, action]
						}
					},
					n12[as={Place banana at $p_{1}$ in bowl}, action]
				}
			},
			n13[as={In gripper none?}, condition],
			n14[as={Gripper open?}, condition]
		}
	};
\end{tikzpicture}
